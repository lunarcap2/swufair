...
<%
	...
	Dim title, explain, mentoring_day, mentoring_time, img_logo, link_url, s_date, s_time, e_date, e_time, limit_cnt, mentoring_gubun
	'limit_cnt = 4
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		$("#mentoring_stime").val(arr[0]);
		$("#mentoring_etime").val(arr[1]);
		...
		*/
		...
		var mentoring_gubun = "<%=mentoring_gubun%>";
		$('[name="mentoring_gubun"]').each(function() {
			if (mentoring_gubun.indexOf(this.value) >= 0)
				this.checked = true;
		});
	});
	...
	function fn_submit() {
		...
		*/
		...
		if ($("#mentoring_stime").val() == "" || $("#mentoring_etime").val() == "") {
			alert("직무상담일자 - 시간을 선택해 주세요.");
			return;
		}
	...
	<div class="content">
		<div class="navi">
			<ul>
				...
				<% If mode = "edit" Then %>
				<li><a href="#none;">현직자 특강&상담 정보 관리 수정</a></li>
				<% Else %>
				<li><a href="#none;">현직자 특강&상담 정보 관리 등록</a></li>
				<% End If %>
			</ul>
		</div>
		<div class="list_area view">
			<div class="tit">
				<% If mode = "edit" Then %>
				<h3>현직자 특강&상담 정보 관리 수정</h3>
				<% Else %>
				<h3>현직자 특강&상담 정보 관리 등록</h3>
				<% End If %>
			</div>
			<table class="tb">
				<colgroup>
					...
					<tr>
						...
						<td colspan="3">
							...
							<span class="selectbox" style="width:210px;">
								<span class="">선택</span>
								<select id="mentoring_stime" name="mentoring_stime" title="시간선택">
							...
							<span class="selectbox" style="width:210px;">
								<span class="">선택</span>
								<select id="mentoring_etime" name="mentoring_etime" title="시간선택">
									<option value="">선택</option>
									<% For i=0 To 23 %>
									<option value="<%=i%>"><%=i%>시</option>
									<% Next %>
						...
						<td colspan="3" class="input_add">
							<div class="rdi_box">
								<label class="radiobox">
									<input type="radio" class="rdi" name="mentoring_gubun" value="기업설명회" checked>
									<span>기업설명회</span>
								</label>
								<label class="radiobox">
									<input type="radio" class="rdi" name="mentoring_gubun" value="동문설명회">
									<span>동문설명회</span>
								</label>
							</div>
						</td>
